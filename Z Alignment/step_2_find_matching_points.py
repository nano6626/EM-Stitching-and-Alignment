...
load_dir = '' #directory where you want to load images from
...
def get_pairs(start, end, depth):
    '''
    Gets the possible pairs of images for mpt calculations
    ...
    start, end, depth : int
                        ...
                        end is the final index
                        depth is how far along the stack you want to compare images
                        for example, depth = 2 means you compare image i with i-2, i-1, i+1 and i+2
    '''
...
def find_mask(image):
    '''finds a mask of the worm, can help when you have a lot of artifacts outside of your region of interest'''
...
def generate_mask(img, tlc, brc):
    '''creates a mask when given a top left corner (tlc) and bottom right corner (brc)'''
...
def find_nbhd(target_img, x, y, epsilon):
    '''finds the neighbourhood of a point'''
...
def node_in_mask(node, source_mask):
    '''checks if the node is in the mask'''
...
def filters(offset_x, offset_y, pr, mesh_thresh, min_pr):
    '''not needed yet, these are various filters including a displacement filter, a nan filter and
    a peak ratio filter'''
...
def pairwise_matching(img1, img2, mask_corners1, mask_corners2, nbhd_size, num_of_nodes, mesh_thresh, min_pr):
    '''finds matching points between pairs of images, filters them according to parameters'''
...
def find_matching_points(start, end, num_of_nodes, nbhd_size, mesh_thresh, pr):
    ...
    for pair in pairs:
        ...
        mask_corners1 = [[1,1],[19999,19999]]
        mask_corners2 = [[1,1],[19999,19999]]
...
mpts = find_matching_points(start, end, 200, 100, 100, 1.7)

savedir = ''
np.save(savedir + 'mpts_0_540.npy', mpts)